...
- 当前账号与登录/退出入口。
- Phase 5B 登录后先展示本地/云端只读摘要，并明确“尚未启用同步”；此步骤不得自动上传。
- 同步是否启用、当前状态、待同步数量和冲突数量。
- 上次成功同步时间、手动重试和暂停同步。
- 首次本地数据迁移入口及结果。
- “清除此设备数据”独立危险操作；存在未同步修改时默认阻断。
- JSON 备份说明：云同步用于多设备一致，导出文件用于用户持有的独立恢复副本。
