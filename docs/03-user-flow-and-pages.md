...
首次访问先进入无路由数据泄露的访问密码页；会话验证并加载云端快照后默认进入 `/today`。首次使用且云端没有任务时，通过空状态引导创建第一项任务。
...
密码错误、限流、断网或数据库失败必须停留在明确的错误/重试状态，不得回退为一个看似可写的本地模式。

### FLOW-CLOUD-02 云端写入（当前）

```text
用户创建或修改实体
  → 客户端发送当前 revision 和新数据
  → 服务端验证会话、来源、数据结构和领域约束
  → Neon 事务成功
  → API 返回新 revision
  → 客户端更新内存快照和派生页面
```

如果写入失败，界面保留旧状态并提示重试；如果 revision 过期，重新加载云端数据并提示该记录已在别处更新。

### FLOW-CLOUD-03 迁移既有浏览器数据（当前）
...
首次解锁
  ...
  → 服务端校验并在一个事务中写入 Neon
  → 从 Neon 重新加载
  → 保留旧 IndexedDB，直到用户自行确认不再需要
```

以下 FLOW-07～09 是 D-014～018 的历史同步流程，已由 D-020 取代，不再作为当前交互目标。
...
### FLOW-07 启用账号与首次云同步（历史）

```mermaid
flowchart TD
    A["在 Settings 选择启用云同步"] --> B["阅读数据上传与隐私说明"]
    B --> C["登录或创建个人账号"]
    C --> D["统计本地与云端数据"]
    D --> E{"两端数据状态"}
    E -- "云端空、本地有数据" --> F["展示本地摘要并建议导出备份"]
    F --> G{"确认首次上传？"}
    G -- "否" --> H["继续仅本地使用"]
    G -- "是" --> I["幂等上传完整快照"]
    I --> J{"服务端确认完整？"}
    J -- "否" --> K["保留本地数据并允许重试"]
    J -- "是" --> L["标记同步已启用并开始增量同步"]
    E -- "本地空、云端有数据" --> M["拉取云端数据到本地"]
    E -- "两端均空" --> L
    E -- "两端均有数据" --> N["停止自动处理，展示安全选项"]
```

规则：

- 登录成功不等于同意上传；首次上传需要单独确认。
- 自动首次上传只处理“云端为空”的账号，不能用本地快照静默覆盖已有云端数据。
- “两端均有数据”时不自动合并，也不提供未经验证的“一键覆盖”；用户先导出本地备份，再按后续明确流程处理。
- 迁移过程中本地库仍是可用数据源；失败、取消或关闭页面都不能清除它。

### FLOW-08 离线修改与恢复同步（Phase 5）

```text
创建或编辑任务
  → 同一 IndexedDB 事务写入业务数据与待同步操作
  → 列表立即更新
  → 若离线，显示“已保存在本设备，等待同步”
  → 恢复网络且会话有效
  → 幂等推送待同步操作
...
同步失败时保留队列；“同步中”“已同步”只能由真实状态驱动，不能由网络请求开始或乐观 UI 推断。

### FLOW-09 处理多设备冲突（Phase 5）

```text
设备提交带 baseRevision 的修改
  → 服务端发现 revision 已变化并拒绝覆盖
  → 客户端保存本地版本和云端版本
  → 全局状态与 Settings 显示冲突数量
  → 用户查看字段差异
  → 明确选择保留本地版本或云端版本
  → 以最新 revision 提交解决结果
  → 两端拉取一致版本
...
Phase 5 不做静默“最后写入获胜”；删除与编辑冲突也必须保留可判断的信息。
...
- 当前会话状态、云端连接状态和退出入口。
- 云端任务、项目、标签和已完成数量摘要。
- 仅在云端为空且旧 IndexedDB 有数据时显示一次性迁移入口及结果。
- JSON 备份说明：Neon 是在线事实来源，导出文件是用户持有的独立恢复副本。
...
- 已启用同步且离线：提示“已保存在本设备，联网后同步”，不得仅显示模糊的“保存成功”。
- 同步失败：保留本地修改，显示待同步数量、简化错误原因和重试入口。
- 会话失效：提示重新登录；不丢弃本地修改或待同步队列。
- 冲突状态：显示冲突数量并提供处理入口；不得用普通错误 Toast 代替。
...
- 不用羞辱或制造焦虑的逾期文案。
- 同步文案区分“已保存在本设备”“等待同步”“已同步到云端”，避免让用户误判数据位置。
